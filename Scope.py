...
def drawControls():
    ...
    drawWords("1/"+unichr(0x394)+"Time",540,257,black,backCol)
    drawWords(unichr(0x394)+"Time",540,237,black,backCol)
    drawWords("Saved Time",540,217,black,backCol)
    drawWords("Time",540,197,black,backCol)
    drawWords(unichr(0x394)+"Voltage",540,167,black,backCol)
...
def updateControls(blank):
    ...
    volts_cm = int(0.0048828125*128*(1000/expandV))
    ...
    if measureTime :
       ...
       drawWords(" "+trunk(t,5)+" "+unichr(0x3bc)+"S",640,197,black,pramCol) # current time        
       drawWords(" "+trunk(savedTime,5)+" "+unichr(0x3bc)+"S",640,217,black,pramCol)
       drawWords(" "+trunk(t-savedTime,5)+" "+unichr(0x3bc)+"S",640,237,black,pramCol) # delta time
       if t-savedTime != 0 :
          drawWords((trunk(1000000 / abs(t-savedTime),5))+" Hz",640,257,black,pramCol)
    if measureVolts :
       #vDisp = (((1024-cursorV)>>2)-128)*volts_sample * vMag
       vDisp = (((1024-cursorV)>>2)-128)*0.0048828125 * vMag
